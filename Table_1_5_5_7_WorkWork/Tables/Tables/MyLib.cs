...
namespace Tables
{
    ...
    public static class MyLib
    {
        ...
        */
        ...
        public static void GetValsFromStringByNames(string where, string[] names, ref string[]vals, ref bool correct)
        {
            int namePosN = 0;
            int Q=names.Length;
            int count=0;
            ...
            for (int i = 1; i <= Q; i++)
            {
                namePosN = FirstSubstringPosN(where, names[i-1]);
                if (namePosN > 0)
                {
                    count++;
                }
            ...
            {
                for (int i = 1; i <= Q-1; i++){
                    curVal = SubstringBetweenSubstrings(where, names[i - 1], names[i + 1 - 1]);
                    MyLib.AddToVector(ref vals, ref count, curVal);
                }
                curVal = SubstringBetweenSubstrings(where, names[Q - 1]);
                MyLib.AddToVector(ref vals, ref count, curVal);
            }
        }
    }//cl
