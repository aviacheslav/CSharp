...
namespace Tables
{
    ...
    public class DataTypeRow
    {
        ...
        public DataCellTypeInfo GetTypeCellN(int CellN)
        {
            return row[CellN - 1];
        }
        public int GetTypeN(int CellN)
        {
            return row[CellN - 1].GetTypeN();
        }
        public int GetLength(int CellN)
        {
            return row[CellN - 1].GetLength();
        }
        public void GetTypeInfoOfCellN(int CellN, ref int TypeN, ref int length)
        {
            TypeN = row[CellN - 1].GetTypeN();
            length = row[CellN - 1].GetLength();
        }
    }//cl _TypesRow
    public class DataTypeTable
    {
        ...
        public DataTypeTable(int QLines, int QColumns, bool LinesNotColumnsHaveSameTypes, int[] types, int length = 2)
        {
            int TypeN = TableConsts.DefaultContentCellTypeN;
            //int length = 2;
            this.cells = new DataCellTypeInfo[QLines][];
            for (int i = 1; i <= QLines; i++)
            {
                ...
                for (int j = 1; j <= QColumns; j++)
                {
                    ...
                    {
                        if (types != null && TableConsts.TypeNIsCorrect(types[j - 1])) TypeN = types[j - 1];
                        //if (lengths != null && lengths[i - 1] > 0 && lengths[i - 1] < MyLib.MaxInt) length = lengths[j - 1];
                    }
                    this.cells[i - 1][j - 1] = new DataCellTypeInfo(TypeN, length);
                }
    ...
    public class TableCellsTypeMap
    {
        ...
        {
            int R = 0;
            R = ColOfLineHeader[LineN - 1].GetTypeN();
            return R;
        }
