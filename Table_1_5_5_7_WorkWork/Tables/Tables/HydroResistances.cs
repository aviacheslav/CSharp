...
namespace Tables
{
    ...
    public class HydroPipelineElement : ICloneable
    {
        ...
        public void AddInner(HydroPipelineElement elem, TValsShowHide vsh=null)
        {
            ...
            //
            this.curSubElementN = this.QElements; // cur = selected
            //
        ...
        public void ShowCoords_IpseAndSub(TValsShowHide vsh, TValsShowHide vshInner=null)
        {
            string s = this.CoordsToStr(vshInner);
            MyLib.writeln(vsh, s);
            for (int i = 1; i <= this.QElements; i++)
            {
                this.elems[i - 1].ShowCoords_IpseAndSub(vsh, vshInner);
            }
        }
        ...
        public void Set_Gv(double val)
        {
            this.HydroRData.IniData.Gv = val;
        }
        public double Get_Gv()
        {
            return this.HydroRData.IniData.Gv;
        }
        ...
        //
        public double Calc_k_by_G(double G, int methodN=1,TValsShowHide vsh=null)
        {
            double k=0, cur_element_k;
            double[] G_each, k_each, part_each, numerator_each=null, denominator_each=null;
            double denom_summand = 1, numerator = 1, denominator = 0, part = 1;
            MyLib.writeln(vsh, "Calc_k_by_G starts working: "+this.ToString());
            MyLib.writeln(vsh, "G="+G.ToString());
            if (this.elems == null)
            {
                MyLib.writeln(vsh, "No sub-elemsg");
                this.Set_Gv(G);
                //k = this.Calc_k_by_Type(); //vikt: to do!
                k = this.HydroRData.IniData.kG; ///vikt: nur tempy!
            }
            else
            {
                G_each=new double[this.QElements];
                k_each=new double[this.QElements];
                part_each=new double[this.QElements];
                //numerator_each = new double[this.QElements];
                //denominator_each = new double[this.QElements];
                //
                //vrn1
                if(this.Conn_Suc0Par1==0){
                    MyLib.writeln(vsh, "Connection type: succ.");
                    for(int i=1; i<=this.QElements; i++){
                        G_each[i-1]=G;
                        //elems[i-1].Set_Gv(G_each[i-1]);
                        k_each[i-1]=elems[i-1].Calc_k_by_G(G_each[i-1]);
                        k+=k_each[i-1]*k_each[i-1];
                        //
                        MyLib.writeln(vsh, i.ToString() + " G[i]=" + G_each[i - 1].ToString() + " k[i]=" + k_each[i - 1].ToString() + " now k=" + k.ToString());
                    }
                    k=Math.Sqrt(k);
                //}else if(this.QElements==1){//also needed
                //}else if(this.QElements==2){//also needed
                }else{
                    if (methodN == 1)
                    {
                        //step 1 of 2
                        MyLib.writeln(vsh, "Connection type: parallel");
                        MyLib.writeln(vsh, "Step 1 of 2. Calc parts of G");
                        for (int eltN = 1; eltN <= this.QElements; eltN++)
                        {
                            G_each[eltN - 1] = G;
                            k_each[eltN - 1] = elems[eltN - 1].Calc_k_by_G(G_each[eltN - 1]);
                            MyLib.writeln(vsh, eltN.ToString() + " G[i]=" + G_each[eltN - 1].ToString() + " k[i]=" + k_each[eltN - 1].ToString());
                            for (int i = 1; i <= this.QElements; i++)
                            {
                                denom_summand = 1;
                                MyLib.writeln(vsh, i.ToString() + "forming denominator");
                                for (int j = 1; j <= i - 1; j++)
                                {
                                    denom_summand *= k_each[j - 1];
                                    MyLib.writeln(vsh, j.ToString() + " k[i]=" + k_each[j - 1].ToString() + "denom_summand now =" + denom_summand.ToString());
                                }
                                MyLib.writeln(vsh, i.ToString() + " missed");
                                for (int j = i + 1; j <= this.QElements; j++)
                                {
                                    denom_summand *= k_each[j - 1];
                                    MyLib.writeln(vsh, j.ToString() + " k[i]=" + k_each[j - 1].ToString() + "denom_summand now =" + denom_summand.ToString());
                                }
                                numerator *= k_each[i - 1];
                                denominator += denom_summand;
                                MyLib.writeln(vsh, i.ToString() + " k[i]=" + k_each[i - 1].ToString() + "numerator now =" + numerator.ToString() + "denominator now =" + denominator.ToString());
                            }
                            if (denominator == 0)
                            {
                                MyLib.writeln(vsh, " denominator == 0 (and it's error!) =" + denominator.ToString());
                                part_each[eltN - 1] = numerator;
                                MyLib.writeln(vsh, " part[" + eltN.ToString() + "]= " + part_each[eltN - 1].ToString());
                            }
                            else
                            {
                                part_each[eltN - 1] = numerator / denominator;
                                MyLib.writeln(vsh, " part[" + eltN.ToString() + "]= " + numerator.ToString() + " / " + denominator.ToString() + " = " + part_each[eltN - 1].ToString());
                            }
                        }
                        if (vsh != null)
                        {
                            MyLib.writeln(vsh, "So parts:");
                            for (int eltN = 1; eltN <= this.QElements; eltN++)
                            {
                                MyLib.writeln(vsh, " part[" + eltN.ToString() + "]= " + part_each[eltN - 1].ToString());
                            }
                        ...
                        //step 2 of 2
                        MyLib.writeln(vsh, "Step 2 of 2. Calc k's by calc'd parts of G");
                        for (int eltN = 1; eltN <= this.QElements; eltN++)
                        {
                            G_each[eltN - 1] = part_each[eltN - 1] * G;
                            k_each[eltN - 1] = elems[eltN - 1].Calc_k_by_G(G_each[eltN - 1]);
                            //
                            k += 1 / k_each[eltN - 1];
                            //
                            MyLib.writeln(vsh, eltN.ToString() + " G[i]=" + G_each[eltN - 1].ToString() + " k[i]=" + k_each[eltN - 1].ToString() + " now k=" + k.ToString());
                        }
                        k = 1 / k;
                    }//methodN
                }//if conn par
            }//if Elements exis
            MyLib.writeln(vsh, "Answer k=: " + this.ToString());
            MyLib.writeln(vsh, "Calc_k_by_G finishes working: " + this.ToString());
            return k;
        }//fn
        //
        ...
        public void GetPipelineParamsNames(ref string[]names, ref int QNames)
        {
            //QNames = 6;
            //names = new string[QNames];
            //names[1 - 1] = "Gv";
            //names[2 - 1] = "Conn.type";
            //names[3 - 1] = "L";
            //names[4 - 1] = "SectIsRound";
            //names[5 - 1] = "d/S";
            //names[6 - 1] = "Surface";
            //
            QNames = 5;
            ...
            names[1 - 1] = "Gv";
            names[2 - 1] = "L";
            names[3 - 1] = "SectIsRound";
            names[4 - 1] = "d/S";
            names[5 - 1] = "Surface";
        }
        public void GetPipelineParamsNamesShort(ref string[] names, ref int QNames)
        {
            QNames = 2;
            names = new string[QNames];
            names[1 - 1] = "Gv";
        ...
        public DataCellRow GetPipelineParamsShort()
        {
            string[]names=null;
            int QItems=0;
            HydroPipelineElement ptr;
            //ptr = this;
            ptr = this.nav_GoToTop();
            ptr.GetPipelineParamsNames(ref names, ref QItems);
            DataCell ComnnTypeCell = new DataCell(new string[] { "--", "//" }, 2);
            ComnnTypeCell.SetActiveN(ptr.Conn_Suc0Par1 + 1);
            //
            DataCellRow row=new DataCellRow(
                new DataCell[]{
                    new DataCell(ptr.HydroRData.IniData.Gv),
                    ComnnTypeCell
                },
                2
            );
        ...
        public DataCellRow GetPipelineParams_AsDataCellRow()
        {
            string[] names = null;
            int QItems = 0;
            HydroPipelineElement ptr;
            ...
            DataCellRow row = new DataCellRow(
                new DataCell[]{
                    new DataCell(ptr.HydroRData.IniData.Gv),
                    ComnnTypeCell,
                    new DataCell(ptr.HydroRData.IniData.De),
                    new DataCell(ptr.HydroRData.IniData.L),
                    new DataCell(0)                                         //need to add - surf charact param for lambda
                },
                5
            );
            return row;
        }
        public TTable hydroResistance_MainResData_GetAsTable()
        {
            TTable tbl = null;
            string[]names=null;
            int QItems=0;
            this.GetPipelineParamsNames/*Short*/(ref names, ref QItems);
            tbl = new TTable(
                new TableInfo(true, true, true, QItems, 1),
                true,
                new DataCellRow[] { this.GetPipelineParams_AsDataCellRow() },
                new DataCellRow(new DataCell[] { new DataCell("Params") }, 1),
                new DataCellRow(names, QItems),
                new TableHeaders(new DataCell("PipeLine params"), new DataCell("Items"), null),
                true
                );
            return tbl;
        }
        public void hydroResistance_MainResData_SetFromTable(TTable tbl)
        {
            HydroPipelineElement ptr;
            //ptr = this;
            ptr = this.nav_GoToTop();
            ptr.Conn_Suc0Par1 = tbl.GetIntVal(2, 1);
            //if (ptr.HydroRData == null)
            //{
            //    ptr.HydroRData = new HydroResistanceData();
            //}
            //if(ptr.HydroRData.IniData==null
            ptr.HydroRData.IniData.Gv = tbl.GetDoubleVal(1, 1);
        }
        //
